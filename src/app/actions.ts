...
export async function processSale(
  ...
  // Basic shape validation (no price from client — server owns pricing)
  if (!items.length) throw new Error('Cart is empty');
  if (items.some(i => i.quantity < 1 || !Number.isInteger(i.quantity))) {
  ...
  return prisma.$transaction(async (tx) => {
    // ── FIX 1: Batch-fetch all products in ONE query (kills N+1) ─────────
    ...
    for (const item of items) {
      ...
      // FIX 2: Timezone-safe expiry check (start of today UTC, not "right now")
    ...
    // ── FIX 3: Server-side pricing — never trust client-supplied price ────
    ...
    const sale = await tx.sale.create({
      data: {
        totalAmount: Math.round(serverTotal * 100) / 100, // round to 2dp
        paymentType: 'Cash',
        status:      'Completed',
        sellerId,
        tenantId,
        ...
        items: {
          create: items.map(item => ({
            ...
            price:     productMap.get(item.id)!.price, // authoritative DB price
          })),
        },
      },
    });
    ...
    // ── FIX 4: Atomic stock decrement — checks stock AGAIN inside the
    //    transaction, preventing race conditions when two cashiers sell
    ...
    for (const item of items) {
      const affected = await tx.product.updateMany({
        where: {
          id:       item.id,
          tenantId,
          stockQty: { gte: item.quantity }, // only update if stock still sufficient
        },
        data: { stockQty: { decrement: item.quantity } },
      });
      if (affected.count === 0) {
        // Another concurrent transaction grabbed the last units — roll back
        throw new Error(
