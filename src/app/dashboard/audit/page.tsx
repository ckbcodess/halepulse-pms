...
// Audit dashboard — read-only view of today's sales + transaction count.
export default async function AuditDashboard() {
  ...
  if (!isImpersonating && role !== 'AUDIT' && role !== 'NES') redirect('/login');

  const tenantId = isImpersonating ? impersonation.tenantId : session.user.tenantId!;

  const ctx = await getTenantContext();
  const hiddenWidgets = Array.from(await getHiddenWidgets(tenantId, isImpersonating ? impersonation.role : role));
