...
export default function ImportPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      {step === 'upload' && (
        <div className="space-y-6">
          <div
            ...
            className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-12 text-center cursor-pointer hover:border-indigo-400 dark:hover:border-indigo-500 hover:bg-indigo-50/50 dark:hover:bg-indigo-500/5 transition-all"
          >
            <Upload size={40} className="mx-auto text-slate-400 dark:text-slate-500 mb-4" />
            <p className="text-lg font-medium text-slate-700 dark:text-slate-300 mb-1">Click to upload CSV</p>
          ...
          <div className="bg-slate-50 dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800">
            ...
            <code className="text-xs text-slate-600 dark:text-slate-400 block bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 overflow-x-auto">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-3">
              ...
              alternate column names like <code className="bg-slate-200 dark:bg-slate-700 px-1 rounded">ITEM_NAME</code>, <code className="bg-slate-200 dark:bg-slate-700 px-1 rounded">RATE_A</code>, <code className="bg-slate-200 dark:bg-slate-700 px-1 rounded">PRATE</code>, <code className="bg-slate-200 dark:bg-slate-700 px-1 rounded">STOCK</code>.
            </p>
            <a href="/import-template.csv" download className="inline-flex items-center gap-2 text-xs text-indigo-600 dark:text-indigo-400 font-medium mt-3 hover:underline">
      ...
      {step === 'preview' && (
        <div className="space-y-6">
          <div className="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-indigo-700 dark:text-indigo-300">{rows.length} products ready to import</p>
              </div>
            </div>
            <button onClick={() => { setStep('upload'); setRows([]); }} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
              Change file
            </button>
          </div>
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {rows.slice(0, 100).map((r, i) => (
                    <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      ...
                      <td className="px-4 py-2 text-right text-slate-600 dark:text-slate-300">{r.price.toFixed(2)}</td>
                      <td className="px-4 py-2 text-right text-slate-600 dark:text-slate-300">{r.stockQty}</td>
                      <td className="px-4 py-2 text-slate-500 dark:text-slate-400 text-xs">{r.category}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {rows.length > 100 && (
              <div className="bg-slate-50 dark:bg-slate-900 px-4 py-2 text-xs text-slate-500 text-center border-t border-slate-200 dark:border-slate-800">
          ...
          <div className="flex gap-3 justify-end">
            <button onClick={() => { setStep('upload'); setRows([]); }} className="px-5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
              Cancel
            </button>
            <button onClick={handleImport} className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold transition-colors">
      ...
      {step === 'importing' && (
        <div className="text-center py-20">
          ...
          <p className="text-lg font-medium text-slate-700 dark:text-slate-300">Importing {rows.length} products...</p>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">This may take a minute for large files</p>
        </div>
      )}
      ...
      {step === 'done' && result && (
        <div className="space-y-6">
          ...
          <div className="flex gap-3 justify-center">
            <button onClick={() => router.push('/inventory')} className="px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold transition-colors">
              Go to Inventory
            </button>
            <button onClick={() => { setStep('upload'); setRows([]); setResult(null); }} className="px-5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
              Import More
            </button>
          </div>
        </div>
