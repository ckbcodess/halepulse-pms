...
export default function BranchesPage() {
  ...
  const FormFields = ({ onSubmit, onCancel }: { onSubmit: (e: React.FormEvent) => void; onCancel: () => void }) => (
    <form onSubmit={onSubmit} className="p-6 space-y-4 border-b border-slate-100 dark:border-white/5 bg-indigo-50/50 dark:bg-indigo-500/5">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Branch Name <span className="text-rose-500">*</span></label>
        ...
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Phone</label>
          <input type="tel" value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+233 XX XXX XXXX"
            className="w-full px-3 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all" />
        </div>
        <div className="sm:col-span-2 space-y-1.5">
          <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Address</label>
      ...
      <div className="flex gap-2">
        ...
        <button type="button" onClick={onCancel}
          className="px-4 py-2 border border-slate-200 dark:border-white/10 rounded-lg text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
          Cancel
        </button>
      </div>
    </form>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">Branches</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{branches.length} branch{branches.length !== 1 ? 'es' : ''}</p>
        </div>
        <button
            ...
            {branches.map(branch => (
              <div key={branch.id}>
                {editingId === branch.id ? (
                  ...
                  <div className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-white/[0.02] group">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-slate-900 dark:text-slate-200">{branch.name}</p>
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${branch.isActive ? 'bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => toggleActive(branch)}
                        className={`p-2 rounded-lg transition-colors ${branch.isActive ? 'hover:bg-rose-50 dark:hover:bg-rose-500/10 text-slate-400 hover:text-rose-600' : 'hover:bg-emerald-50 dark:hover:bg-emerald-500/10 text-slate-400 hover:text-emerald-600'}`}>
                        <Power size={14} />
                      </button>
                    </div>
                  </div>
